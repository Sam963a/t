...
if (isset($_POST['submit'])) {
    if (!empty($_POST['sort']) && !empty($_POST['brand']) && !empty($_POST['type']) && !empty($_POST['size'])
        ...
        if (!$size || !$price || !$stock) {
            ...
            if ($stock > 0) {
                $query = $db->prepare('INSERT INTO schoenen (soort, merk, type, maat, prijs, voorraad) 
                        VALUES (:sort, :brand, :type, :size, :price, :stock)');
                ...
                if ($query->execute()) {
                    $_SESSION['message'] = "De schoen is aangemaakt";
